...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({ problem, _solved }) => {
	...
	const [updating, setUpdating] = useState(false); // This is assume when you click on the like button and the loader start then it would not accept click
	...
	return (
		<div className='bg-dark-layer-deep'>
			{/* TAB */}
			<div className='flex h-11 w-full items-center pt-2 bg-dark-layer-2 overflow-x-hidden'>
				<div className={`bg-dark-layer-1 rounded-t-[5px] mr-2 px-5 py-[6px] text-xs cursor-pointer 
					${!snippet ? "text-white" : "text-gray-500"}
					`}
					onClick={() => setSnippet(false)}
					>
					Description
				</div>
				<div className={`bg-dark-layer-1 rounded-t-[5px] mr-2 px-5 py-[6px] text-xs cursor-pointer 
					${snippet ? "text-white" : "text-gray-500"}
					`}
					onClick={() => setSnippet(true)}
					>
					Snippet
				</div>
			</div>

			{!snippet && (<div className='flex px-0 py-4 h-[calc(100vh-94px)] overflow-y-auto'>
				<div className='px-5'>
					{/* Problem heading */}
	...
	<div className='p-4'>
		{user ? (
			<SnippetEditor userId={user.uid} />
		) : (
			<p className='text-white'>Please sign in to view your saved snippet.</p>
		)}
	</div>
)}
		</div>
...
const SnippetEditor = ({ userId }: { userId: string }) => {
	...
	useEffect(() => {
		const fetchSnippet = async () => {
			try {
				...
				if (userSnap.exists()) {
					const snippet = userSnap.data().snippit;
					if (snippet && snippet.code && snippet.language) {
						setLanguage(snippet.language);
						setCode(snippet.code);
					} else {
						setCode("// No snippet saved.");
					}
				}
			} catch (err) {
	...
	if (loading) return <p className='text-white'>Loading snippet...</p>;

	return (
		<div className='w-full'>
			<p className='text-white mb-2'>
				Showing your saved snippet in <strong>{language}</strong>
			</p>
			<Editor
				height="75vh"
